...
func ToAlbAPIResources(ctx context.Context, namespace string, inputFile string, providers []string) ([]AlbResources, error) {
	...
	for _, provider := range providerByName {
		providerGatewayResources, conversionErrs := provider.ToAlbConfig()
		errs = append(errs, conversionErrs...)
		albResources = append(albResources, providerGatewayResources)
	}
	if len(errs) > 0 {
		return nil, aggregatedErrs(errs)
	}

	return albResources, nil
}
...
// constructProviders constructs a map of concrete Provider implementations by their ProviderName.
